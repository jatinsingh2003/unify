...
// Spend vs Revenue area chart — now fetches real data from /api/metrics/timeseries
...
export function PerformanceChart({ days = 30, platform = "all" }: Props) {
  ...
  if (!data.length) {
    return (
      <div className="h-64 flex items-center justify-center text-muted-foreground text-sm">
        No data yet — connect a platform and run a sync.
      </div>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <AreaChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
        ...
        />
        <YAxis tickFormatter={fmt} tick={{ fontSize: 12 }} className="text-muted-foreground" />
        <Tooltip
