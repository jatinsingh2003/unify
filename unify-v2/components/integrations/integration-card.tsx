    ...
    <div className="rounded-xl border bg-card p-5 flex flex-col gap-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-muted">{logo}</div>
          <div>
        ...
        <Badge variant={connected ? "default" : "secondary"}
          ...
          {connected
            ? <span className="flex items-center gap-1"><CheckCircle2Icon className="w-3 h-3" /> Connected</span>
            : <span className="flex items-center gap-1"><XCircleIcon className="w-3 h-3" /> Not connected</span>}
        </Badge>
      </div>
      {lastSyncedAt && (
        <p className="text-xs text-muted-foreground">Last synced: {new Date(lastSyncedAt).toLocaleString()}</p>
      )}
      {syncMsg && <p className="text-xs text-muted-foreground bg-muted rounded p-2">{syncMsg}</p>}
      <div className="flex gap-2 mt-auto">
        {connected ? (
          <Button size="sm" variant="outline" onClick={handleSync} disabled={syncing} className="w-full">
            {syncing ? <Loader2Icon className="w-3 h-3 mr-2 animate-spin" /> : <RefreshCwIcon className="w-3 h-3 mr-2" />}
            Sync Now
          </Button>
        ) : (
          <Button size="sm" className="w-full" asChild><a href={connectHref}>Connect</a></Button>
        )}
      </div>
    </div>
  );
